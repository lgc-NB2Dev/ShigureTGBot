...
async def _(bot: Bot, matcher: Matcher, event: MessageEvent,
    ...
    if not arg:
        return await matcher.finish('用法：/netease <歌曲名>\n'
                                    '可以听需要会员的歌曲（黑胶为自费）\n'
                                    '球球给点吃的吧~ → /about',
...
async def edit_search_music_msg(bot, msg_id, chat_id, salt, page=1):
    ...
    try:
        ...
        return bot.edit_message_text(text='歌曲搜索失败，请重试',
                                     message_id=msg_id,
                                     chat_id=chat_id)

    if ret['code'] != 200:
        return bot.edit_message_text(text=f'未知错误({ret["code"]})',
                                     message_id=msg_id,
                                     chat_id=chat_id)

    ret = ret['result']
    if not ret['result']['songs']:
        return bot.edit_message_text(text='未搜索到歌曲',
                                     message_id=msg_id,
                                     chat_id=chat_id)
    ...
    for i, song in enumerate(ret['songs']):
        ...
        tmp_row.append(
            InlineKeyboardButton(text=str(num),
...
async def get_music(bot: Bot, music_id, msg_id, chat_id):
    ...
    try:
        ret_down = await get_track_audio([int(music_id)],
                ...
                match data[2]:
                    case 'page':
                        await edit_search_music_msg(
                            bot, event.message.message_id, event.message.chat.id,
                            data[3], data[4]
                        )
                    case 'get':
                        await get_music(bot, data[3],
                                        event.message.message_id, event.message.chat.id)
